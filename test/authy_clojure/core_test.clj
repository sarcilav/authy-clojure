...
(defonce api-key "7ea6e01f516b0a3ba8e9df75d1f9a6f6")
...
(defonce user-response @(register-user {:user {:email "somedude@example.com" :cellphone "333-333-4444" :country_code "57"}} api-key))
...
(deftest test-request-sms
  (testing "request-sms"
    (let [response @(request-sms ((:user user-response) "id") api-key)]
...
(deftest test-request-call
  (testing "request-call"
    (let [response @(request-call ((:user user-response) "id") api-key)]
      (is (= (:status response) 503))
      (is (= (:message response)
...
(deftest test-delete-user
  (testing "delete-user"
    (let [response @(delete-user ((:user user-response) "id") api-key)]
      (is (= (:status response) 200))
      (is (= (:message response) "User was added to remove.")))))

(deftest test-app-details
  (testing "app-details"
    (let [response @(app-details api-key)]
      (is (= (:status response) 200))
...
(deftest test-app-stats
  (testing "app-stats"
    (let [response @(app-stats api-key)]
      (is (= (:status response) 200))
